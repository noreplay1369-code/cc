...
class Openpos_Admin{
    ...
    public function init()
    {
        ...
        // Persist the "per page" screen option for the native POS list tables.
        add_filter( 'set-screen-option', array($this,'op_set_screen_option'), 10, 3 );
        add_filter( 'set_screen_option_op_transactions_per_page', array($this,'op_set_screen_option'), 10, 3 );
        add_filter( 'set_screen_option_op_orders_per_page', array($this,'op_set_screen_option'), 10, 3 );
        add_filter( 'set_screen_option_op_products_per_page', array($this,'op_set_screen_option'), 10, 3 );
...
//        add_action( 'wp_ajax_openpos_report_data_table', array($this,'report_data_table') );
    ...
    public function admin_style() {
        ...
        $allow_bootstrap = array('openpos-dasboard','op-reports','op-registers','op-warehouses','op-stock','op-setting','op-tables','op-receipt-template');
    ...
    function pos_admin_menu() {
        ...
        add_action( 'admin_print_styles-'. $page, array( &$this, 'admin_enqueue' ) );
        add_action( 'load-'. $page, array( $this, 'orders_screen_options' ) );
        ...
        add_action( 'admin_print_styles-'. $page, array( &$this, 'admin_enqueue' ) );
        add_action( 'load-'. $page, array( $this, 'transactions_screen_options' ) );
        ...
        add_action( 'admin_print_styles-'. $page, array( &$this, 'admin_enqueue' ) );
        add_action( 'load-'. $page, array( $this, 'products_screen_options' ) );
        add_action( 'load-'. $page, array( $this, 'products_handle_bulk' ) );
    ...
    function products_page() {
        ...
        require_once( OPENPOS_DIR.'includes/admin/class-op-products-list-table.php' );
        $list_table = new OP_Products_List_Table( $this->core, $this->settings_api, $this );
        $list_table->prepare_items();
        ?>
        <style type="text/css">
            /* ---- WooCommerce-style skin for POS Products list ---- */
            .op-wc-products .wp-list-table { border: 1px solid #e0e0e0; border-radius: 8px; border-collapse: separate; border-spacing: 0; overflow: hidden; }
            .op-wc-products .wp-list-table thead th, .op-wc-products .wp-list-table tfoot th {
                background: #fff; border-bottom: 1px solid #e0e0e0; padding: 12px 14px;
                font-size: 11px; text-transform: uppercase; letter-spacing: .03em; color: #757575; font-weight: 600;
            }
            .op-wc-products .wp-list-table tbody td { padding: 12px 14px; vertical-align: middle; border-top: 1px solid #f0f0f1; }
            .op-wc-products .wp-list-table tbody tr:hover td { background: #f9f7fc; }

            /* checkbox column alignment + WC accent */
            .op-wc-products .wp-list-table th.check-column,
            .op-wc-products .wp-list-table td.check-column {
                width: 40px; padding: 0; text-align: center; vertical-align: middle;
            }
            .op-wc-products .wp-list-table thead td.check-column,
            .op-wc-products .wp-list-table thead th.check-column { padding-top: 0; }
            .op-wc-products .wp-list-table .check-column input[type="checkbox"] {
                float: none; margin: 0; vertical-align: middle;
            }
            .op-wc-products .wp-list-table input[type="checkbox"] {
                width: 18px; height: 18px; border-radius: 4px; border: 1px solid #c3c4c7; accent-color: #7f54b3; cursor: pointer;
            }
            .op-wc-products .wp-list-table input[type="checkbox"]:checked { background: #7f54b3; border-color: #7f54b3; }
            .op-wc-products .wp-list-table input[type="checkbox"]:checked::before { color: #fff; margin: -1px 0 0 -1px; }
            .op-wc-products .wp-list-table input[type="checkbox"]:focus { border-color: #7f54b3; box-shadow: 0 0 0 1px #7f54b3; }
            .op-wc-products .wp-list-table .sorting-indicators .sorting-indicator { }
            .op-wc-products th.sortable a, .op-wc-products th.sorted a { color: #50575e; }

            /* ID + title */
            .op-wc-products td.column-id { font-weight: 600; color: #50575e; }
            .op-wc-products td.column-post_title { color: #1e1e1e; font-weight: 500; }
            .op-wc-products td.column-post_title p { margin: 2px 0 0; color: #757575; font-weight: 400; font-size: 12px; }

            /* thumbnail */
            .op-wc-products .column-product_thumb { width: 64px; }
            .op-wc-products .column-product_thumb img {
                width: 44px; height: 44px; object-fit: cover; border-radius: 6px; border: 1px solid #e0e0e0; display: block; background: #fff;
            }

            /* barcode input */
            .op-wc-products .column-barcode { width: 210px; }
            .op-wc-products .op-barcode-input {
                width: 100%; max-width: 190px; border: 1px solid #c3c4c7; border-radius: 4px; padding: 6px 10px;
                font-size: 13px; min-height: 34px; box-shadow: none; background: #fff; color: #2c3338; font-family: Menlo, Consolas, monospace;
            }
            .op-wc-products .op-barcode-input:focus { border-color: #7f54b3; box-shadow: 0 0 0 1px #7f54b3; outline: none; }

            /* action buttons */
            .op-wc-products .action-row { display: flex; flex-wrap: wrap; gap: 6px; }
            .op-wc-products .action-row a {
                display: inline-flex; align-items: center; gap: 4px; padding: 5px 12px; border: 1px solid #c3c4c7; border-radius: 4px;
                background: #f6f7f7; color: #2c3338; font-size: 12px; font-weight: 600; text-decoration: none; line-height: 1.5; white-space: nowrap;
                transition: background .15s ease, border-color .15s ease, color .15s ease;
            }
            .op-wc-products .action-row a:hover { background: #f0eaf8; color: #7f54b3; border-color: #7f54b3; }
            .op-wc-products .action-row a.print-barcode-product-btn { background: #7f54b3; border-color: #7f54b3; color: #fff; }
            .op-wc-products .action-row a.print-barcode-product-btn:hover { background: #6b4794; border-color: #6b4794; color: #fff; }

            /* toolbar: search + bulk + apply -> WC accent */
            .op-wc-products .search-box input[type="search"], .op-wc-products .tablenav select {
                border: 1px solid #c3c4c7; border-radius: 4px; min-height: 34px; box-shadow: none;
            }
            .op-wc-products .search-box input[type="search"]:focus, .op-wc-products .tablenav select:focus { border-color: #7f54b3; box-shadow: 0 0 0 1px #7f54b3; }
            .op-wc-products .button:not(.button-primary) { border-radius: 4px; }
            .op-wc-products .search-box .button, .op-wc-products #doaction, .op-wc-products #doaction2 {
                background: #7f54b3; border-color: #7f54b3; color: #fff; border-radius: 4px;
            }
            .op-wc-products .search-box .button:hover, .op-wc-products #doaction:hover, .op-wc-products #doaction2:hover {
                background: #6b4794; border-color: #6b4794; color: #fff;
            }
            .op-wc-products .notice { border-radius: 4px; }
        </style>
        <div class="wrap op-wc-products">
            <h1 class="wp-heading-inline"><?php echo esc_html__( 'POS Products', 'openpos' ); ?></h1>
            <hr class="wp-header-end">
            <?php if ( isset( $_GET['op_updated'] ) ) : ?>
                <div class="notice notice-success is-dismissible"><p><?php echo esc_html__( 'Saved', 'openpos' ); ?></p></div>
            <?php endif; ?>
            <form method="get">
                <input type="hidden" name="page" value="op-products" />
                <?php
                    $list_table->search_box( __( 'Search', 'openpos' ), 'op-product' );
                    $list_table->display();
                ?>
            </form>
        ...
        <script type="text/javascript">
            (function(){
                var OP_PH = '<?php echo 'data:image/svg+xml;base64,' . base64_encode( '<svg xmlns="http://www.w3.org/2000/svg" width="44" height="44"><rect width="44" height="44" rx="6" fill="#f0f0f1"/><path d="M13 29l6-7 4 5 3-4 5 6z" fill="#c3c4c7"/><circle cx="16" cy="15" r="3" fill="#c3c4c7"/></svg>' ); ?>';
                function opSwap(img){ img.setAttribute('data-op-ph','1'); img.removeAttribute('srcset'); img.removeAttribute('sizes'); img.src = OP_PH; }
                function opFix(img){ if ( img.getAttribute('data-op-ph') !== '1' && img.complete && img.naturalWidth === 0 ) { opSwap(img); } }
                document.addEventListener('error', function(e){
                    var t = e.target;
                    if ( t && t.tagName === 'IMG' && t.getAttribute('data-op-ph') !== '1' && t.closest && t.closest('.op-wc-products') ) {
                        opSwap(t);
                    }
                }, true);
                function opScan(){ document.querySelectorAll('.op-wc-products img').forEach(opFix); }
                opScan(); setTimeout(opScan, 400); setTimeout(opScan, 1200);
            })();
        </script>
        <?php
    }
    /**
     * Process the Products list-table bulk actions (Save barcodes / Print barcodes)
     * before any output so we can redirect (PRG). Hooked on load-{page}.
     */
    public function products_handle_bulk() {
        $action = '';
        ...
        if ( ! in_array( $action, array( 'save', 'print' ), true ) ) {
            return;
        }
        check_admin_referer( 'bulk-op_products' );
        if ( ! current_user_can( 'manage_woocommerce' ) ) {
            return;
        }

        $ids = isset( $_REQUEST['product'] ) ? array_map( 'intval', (array) $_REQUEST['product'] ) : array();
        if ( empty( $ids ) ) {
            wp_safe_redirect( add_query_arg( array( 'page' => 'op-products' ), admin_url( 'admin.php' ) ) );
            exit;
        }
        ...
        if ( 'save' === $action ) {
            ...
            $barcodes = isset( $_REQUEST['barcode'] ) ? (array) $_REQUEST['barcode'] : array();
            foreach ( $ids as $id ) {
                if ( isset( $barcodes[ $id ] ) && '' !== $barcodes[ $id ] && 'post_id' !== $barcode_field ) {
                    update_post_meta( $id, $barcode_field, sanitize_text_field( wp_unslash( $barcodes[ $id ] ) ) );
                }
            }
            wp_safe_redirect( add_query_arg( array( 'page' => 'op-products', 'op_updated' => 1 ), admin_url( 'admin.php' ) ) );
            exit;
        }

        if ( 'print' === $action ) {
            $url = admin_url( 'admin-ajax.php?action=print_barcode&op_nonce=' . wp_create_nonce( 'op_nonce' ) . '&is_print=1&product_id=' . implode( ',', $ids ) );
            wp_redirect( $url );
            exit;
        }
    }
    public function products_screen_options()
    {
        add_screen_option( 'per_page', array(
            'label'   => __( 'Products per page', 'openpos' ),
            'default' => 20,
            'option'  => 'op_products_per_page',
        ) );
    }
    ...
    public function dashboard_ajax()
    {
        ...
        foreach($pie_data as $p){
            ...
            $registe_data['labels'][] = html_entity_decode(esc_html($p['label']));
        ...
        // Aggregated KPIs for the WooCommerce-Analytics style performance cards.
        $performance_order_count = count( $orders );
        $performance_total_sales = array_sum( $sale_data );
        $performance_total_profit = array_sum( $commision_data );
        $performance_avg_order = $performance_order_count > 0 ? ( $performance_total_sales / $performance_order_count ) : 0;
        ...
        $result = array(
            ...
            'performance' => array(
                'total_sales' => wc_price( $performance_total_sales ),
                'net_profit'  => wc_price( $performance_total_profit ),
                'orders'      => number_format_i18n( $performance_order_count ),
                'avg_order'   => wc_price( $performance_avg_order ),
                'cash_flow'   => wc_price( $total_transaction ),
            ),
        );
    ...
    {
        require_once( OPENPOS_DIR.'includes/admin/class-op-cashiers-list-table.php' );
        $list_table = new OP_Cashiers_List_Table();
        $list_table->prepare_items();
        $op_cashier_nonce = wp_create_nonce( 'op_nonce' );
        ?>
        <style type="text/css">
            /* ---- WooCommerce-style skin for POS Cashiers list ---- */
            .op-wc-cashiers .subsubsub { margin: 8px 0 6px; }
            .op-wc-cashiers .subsubsub a { color: #50575e; }
            .op-wc-cashiers .subsubsub a.current { color: #7f54b3; font-weight: 600; }
            .op-wc-cashiers .wp-list-table { border: 1px solid #e0e0e0; border-radius: 8px; border-collapse: separate; border-spacing: 0; overflow: hidden; }
            .op-wc-cashiers .wp-list-table thead th, .op-wc-cashiers .wp-list-table tfoot th {
                background: #fff; border-bottom: 1px solid #e0e0e0; padding: 12px 14px;
                font-size: 11px; text-transform: uppercase; letter-spacing: .03em; color: #757575; font-weight: 600;
            }
            .op-wc-cashiers .wp-list-table tbody td { padding: 12px 14px; vertical-align: middle; border-top: 1px solid #f0f0f1; }
            .op-wc-cashiers .wp-list-table tbody tr:hover td { background: #f9f7fc; }
            .op-wc-cashiers th.sortable a, .op-wc-cashiers th.sorted a { color: #50575e; }

            /* checkbox column */
            .op-wc-cashiers .wp-list-table th.check-column, .op-wc-cashiers .wp-list-table td.check-column {
                width: 40px; padding: 0; text-align: center; vertical-align: middle;
            }
            .op-wc-cashiers .wp-list-table .check-column input[type="checkbox"] { float: none; margin: 0; vertical-align: middle; }
            .op-wc-cashiers .wp-list-table input[type="checkbox"] {
                width: 18px; height: 18px; border-radius: 4px; border: 1px solid #c3c4c7; accent-color: #7f54b3; cursor: pointer;
            }
            .op-wc-cashiers .wp-list-table input[type="checkbox"]:checked { background: #7f54b3; border-color: #7f54b3; }
            .op-wc-cashiers .wp-list-table input[type="checkbox"]:checked::before { color: #fff; margin: -1px 0 0 -1px; }
            .op-wc-cashiers .wp-list-table input[type="checkbox"]:focus { border-color: #7f54b3; box-shadow: 0 0 0 1px #7f54b3; }

            /* cells */
            .op-wc-cashiers td.column-id { font-weight: 600; color: #50575e; }
            .op-wc-cashiers td.column-name strong { color: #1e1e1e; }
            .op-wc-cashiers .op-user-roles { margin: 2px 0 0; font-size: 11px; color: #a7aaad; text-transform: capitalize; }
            .op-wc-cashiers td.column-user_email a { color: #7f54b3; text-decoration: none; }
            .op-wc-cashiers td.column-user_email a:hover { text-decoration: underline; }

            /* POS staff toggle switch */
            .op-wc-cashiers .op-switch { display: inline-flex; align-items: center; gap: 9px; cursor: pointer; user-select: none; }
            .op-wc-cashiers .op-switch input { position: absolute; opacity: 0; width: 0; height: 0; }
            .op-wc-cashiers .op-switch__track {
                position: relative; width: 40px; height: 22px; border-radius: 22px; background: #c3c4c7; transition: background .18s ease; flex: 0 0 auto;
            }
            .op-wc-cashiers .op-switch__knob {
            ...
            .op-wc-cashiers .op-switch.is-on .op-switch__track { background: #7f54b3; }
            .op-wc-cashiers .op-switch.is-on .op-switch__knob { transform: translateX(18px); }
            .op-wc-cashiers .op-switch__text { font-size: 12px; font-weight: 600; color: #757575; min-width: 24px; }
            .op-wc-cashiers .op-switch.is-on .op-switch__text { color: #2c4700; }
            .op-wc-cashiers .op-switch.op-saving .op-switch__track { opacity: .5; }
            .op-wc-cashiers .op-switch input:focus + .op-switch__track { box-shadow: 0 0 0 2px rgba(127,84,179,.4); }

            /* toolbar + bulk */
            .op-wc-cashiers .search-box input[type="search"], .op-wc-cashiers .tablenav select {
                border: 1px solid #c3c4c7; border-radius: 4px; min-height: 34px; box-shadow: none;
            }
            .op-wc-cashiers .search-box input[type="search"]:focus, .op-wc-cashiers .tablenav select:focus { border-color: #7f54b3; box-shadow: 0 0 0 1px #7f54b3; }
            .op-wc-cashiers .button:not(.button-primary) { border-radius: 4px; }
            .op-wc-cashiers .search-box .button, .op-wc-cashiers #doaction, .op-wc-cashiers #doaction2 {
                background: #7f54b3; border-color: #7f54b3; color: #fff; border-radius: 4px;
            }
            .op-wc-cashiers .search-box .button:hover, .op-wc-cashiers #doaction:hover, .op-wc-cashiers #doaction2:hover {
                background: #6b4794; border-color: #6b4794; color: #fff;
            }
        </style>
        <div class="wrap op-wc-cashiers">
            <h1 class="wp-heading-inline"><?php echo esc_html__( 'POS Cashiers', 'openpos' ); ?></h1>
            <hr class="wp-header-end">
            <?php $list_table->views(); ?>
            <form method="get">
                <input type="hidden" name="page" value="op-cashiers" />
                <?php
                    if ( isset( $_GET['op-display'] ) && $_GET['op-display'] === 'staff' ) {
                        echo '<input type="hidden" name="op-display" value="staff" />';
                    }
                    $list_table->search_box( __( 'Search users', 'openpos' ), 'op-cashier' );
                    $list_table->display();
                ?>
            </form>
        </div>
        <script type="text/javascript">
            (function($){
                "use strict";
                var OP_C_NONCE = '<?php echo esc_js( $op_cashier_nonce ); ?>';
                $(document).on('change', '.op-staff-toggle', function(){
                    var $cb = $(this), uid = $cb.data('user'), on = $cb.prop('checked');
                    var $sw = $cb.closest('.op-switch');
                    $sw.addClass('op-saving');
                    $sw.toggleClass('is-on', on);
                    $sw.find('.op-switch__text').text(on ? '<?php echo esc_js( __( 'Yes', 'openpos' ) ); ?>' : '<?php echo esc_js( __( 'No', 'openpos' ) ); ?>');
                    var data = { action: 'save_cashier', op_nonce: OP_C_NONCE };
                    data['_op_allow_pos[' + uid + ']'] = on ? 1 : 0;
                    $.ajax({
                        url: openpos_admin.ajax_url, type: 'post', dataType: 'json', data: data,
                        complete: function(){ $sw.removeClass('op-saving'); }
                    });
                });
            })(jQuery);
        </script>
    ...
    public function op_set_screen_option( $status, $option, $value )
    {
        $op_options = array( 'op_transactions_per_page', 'op_orders_per_page', 'op_products_per_page' );
        if ( in_array( $option, $op_options, true ) ) {
            return (int) $value;
        }
        return $status;
    }
    public function transactions_screen_options()
    {
        add_screen_option( 'per_page', array(
            'label'   => __( 'Transactions per page', 'openpos' ),
            'default' => 20,
            'option'  => 'op_transactions_per_page',
        ) );
    }
    public function transactions_page()
    {
        ...
        require_once( OPENPOS_DIR.'includes/admin/class-op-transactions-list-table.php' );
        $list_table = new OP_Transactions_List_Table( $this->core, $op_register, $op_warehouse );
        $list_table->prepare_items();
        ?>
        <style type="text/css">
            /* ---- WooCommerce-style skin for Cash Transactions list ---- */
            .op-wc-transactions .subsubsub { margin: 8px 0 6px; }
            .op-wc-transactions .subsubsub a { color: #50575e; }
            .op-wc-transactions .subsubsub a.current { color: #7f54b3; font-weight: 600; }
            .op-wc-transactions .wp-list-table { border: 1px solid #e0e0e0; border-radius: 8px; border-collapse: separate; border-spacing: 0; overflow: hidden; }
            .op-wc-transactions .wp-list-table thead th, .op-wc-transactions .wp-list-table tfoot th {
                background: #fff; border-bottom: 1px solid #e0e0e0; padding: 12px 14px;
                font-size: 11px; text-transform: uppercase; letter-spacing: .03em; color: #757575; font-weight: 600;
            }
            .op-wc-transactions .wp-list-table tbody td { padding: 12px 14px; vertical-align: middle; border-top: 1px solid #f0f0f1; }
            .op-wc-transactions .wp-list-table tbody tr:hover td { background: #f9f7fc; }
            .op-wc-transactions th.sortable a, .op-wc-transactions th.sorted a { color: #50575e; }

            /* checkbox column */
            .op-wc-transactions .wp-list-table th.check-column, .op-wc-transactions .wp-list-table td.check-column {
                width: 40px; padding: 0; text-align: center; vertical-align: middle;
            }
            .op-wc-transactions .wp-list-table .check-column input[type="checkbox"] { float: none; margin: 0; vertical-align: middle; }
            .op-wc-transactions .wp-list-table input[type="checkbox"] {
                width: 18px; height: 18px; border-radius: 4px; border: 1px solid #c3c4c7; accent-color: #7f54b3; cursor: pointer;
            }
            .op-wc-transactions .wp-list-table input[type="checkbox"]:checked { background: #7f54b3; border-color: #7f54b3; }
            .op-wc-transactions .wp-list-table input[type="checkbox"]:checked::before { color: #fff; margin: -1px 0 0 -1px; }
            .op-wc-transactions .wp-list-table input[type="checkbox"]:focus { border-color: #7f54b3; box-shadow: 0 0 0 1px #7f54b3; }

            /* cells */
            .op-wc-transactions td.column-id { font-weight: 600; color: #50575e; }
            .op-wc-transactions td.column-in_amount .woocommerce-Price-amount, .op-wc-transactions td.column-in_amount { color: #2c4700; font-weight: 600; }
            .op-wc-transactions td.column-out_amount .woocommerce-Price-amount, .op-wc-transactions td.column-out_amount { color: #b32d2e; font-weight: 600; }
            .op-wc-transactions td.column-created_at p { margin: 0; }
            .op-wc-transactions td.column-created_at .pos-local-time { font-size: 11px; color: #a7aaad; }

            /* toolbar + bulk */
            .op-wc-transactions .search-box input[type="search"], .op-wc-transactions .tablenav select {
                border: 1px solid #c3c4c7; border-radius: 4px; min-height: 34px; box-shadow: none;
            }
            .op-wc-transactions .search-box input[type="search"]:focus, .op-wc-transactions .tablenav select:focus { border-color: #7f54b3; box-shadow: 0 0 0 1px #7f54b3; }
            .op-wc-transactions .button:not(.button-primary) { border-radius: 4px; }
            .op-wc-transactions .search-box .button, .op-wc-transactions #doaction, .op-wc-transactions #doaction2 {
                background: #7f54b3; border-color: #7f54b3; color: #fff; border-radius: 4px;
            }
            .op-wc-transactions .search-box .button:hover, .op-wc-transactions #doaction:hover, .op-wc-transactions #doaction2:hover {
                background: #6b4794; border-color: #6b4794; color: #fff;
            }
        </style>
        <div class="wrap op-wc-transactions">
            <h1 class="wp-heading-inline"><?php echo esc_html__( 'Cash Transactions', 'openpos' ); ?></h1>
            <hr class="wp-header-end">
            <?php $list_table->views(); ?>
            <form method="get">
                <input type="hidden" name="page" value="op-transactions" />
                <?php
                    if ( ! empty( $_GET['source_type'] ) ) {
                        echo '<input type="hidden" name="source_type" value="' . esc_attr( sanitize_text_field( wp_unslash( $_GET['source_type'] ) ) ) . '" />';
                    }
                    if ( ! empty( $_GET['register'] ) ) {
                        echo '<input type="hidden" name="register" value="' . esc_attr( (int) $_GET['register'] ) . '" />';
                    }
                    if ( ! empty( $_GET['warehouse'] ) ) {
                        echo '<input type="hidden" name="warehouse" value="' . esc_attr( (int) $_GET['warehouse'] ) . '" />';
                    }
                    $list_table->search_box( __( 'Search', 'openpos' ), 'op-transaction' );
                    $list_table->display();
                ?>
            </form>
        </div>
    ...
    {
        add_screen_option( 'per_page', array(
            'label'   => __( 'Orders per page', 'openpos' ),
            'default' => 20,
            'option'  => 'op_orders_per_page',
        ) );
    }
    public function orders_page(){
        ...
        require_once( OPENPOS_DIR.'includes/admin/class-op-orders-list-table.php' );
        $list_table = new OP_Orders_List_Table( $this->core, $op_register, $op_warehouse, $op_woo_order );
        $list_table->prepare_items();
        ?>
        <style type="text/css">
            /* ---- WooCommerce-style skin for POS Orders list ---- */
            .op-wc-orders .wp-list-table { border: 1px solid #e0e0e0; border-radius: 8px; border-collapse: separate; border-spacing: 0; overflow: hidden; }
            .op-wc-orders .wp-list-table thead th, .op-wc-orders .wp-list-table tfoot th {
                background: #fff; border-bottom: 1px solid #e0e0e0; padding: 12px 14px;
                font-size: 11px; text-transform: uppercase; letter-spacing: .03em; color: #757575; font-weight: 600;
            }
            .op-wc-orders .wp-list-table tbody td { padding: 12px 14px; vertical-align: middle; border-top: 1px solid #f0f0f1; }
            .op-wc-orders .wp-list-table tbody tr:hover td { background: #f9f7fc; }
            .op-wc-orders th.sortable a, .op-wc-orders th.sorted a { color: #50575e; }

            /* order number cell */
            .op-wc-orders .op-order-id { margin: 0; font-size: 11px; color: #a7aaad; }
            .op-wc-orders a.op-order-number { color: #7f54b3; font-weight: 600; text-decoration: none; }
            .op-wc-orders a.op-order-number:hover { text-decoration: underline; }
            .op-wc-orders td.column-customer { color: #2c3338; }
            .op-wc-orders td.column-created_by p { margin: 0; font-size: 12px; color: #50575e; }
            .op-wc-orders td.column-created_at p { margin: 0; }
            .op-wc-orders td.column-created_at .pos-local-time { font-size: 11px; color: #a7aaad; }
            .op-wc-orders td.column-total { font-weight: 600; color: #1e1e1e; }

            /* status pills (WooCommerce style) */
            .op-wc-orders .order_status {
            ...
            .op-wc-orders .order_status.completed { background: #c6e1c6; color: #2c4700; }
            .op-wc-orders .order_status.processing { background: #c8d7e1; color: #003a5c; }
            .op-wc-orders .order_status.pending, .op-wc-orders .order_status.pending-payment { background: #f8dda7; color: #573b00; }
            .op-wc-orders .order_status.on-hold { background: #f8dda7; color: #573b00; }
            .op-wc-orders .order_status.refunded { background: #e6e6e6; color: #50575e; }
            .op-wc-orders .order_status.cancelled, .op-wc-orders .order_status.failed { background: #eba3a3; color: #5c0000; }

            /* toolbar */
            .op-wc-orders .search-box input[type="search"], .op-wc-orders .tablenav select {
                border: 1px solid #c3c4c7; border-radius: 4px; min-height: 34px; box-shadow: none;
            }
            .op-wc-orders .search-box input[type="search"]:focus, .op-wc-orders .tablenav select:focus { border-color: #7f54b3; box-shadow: 0 0 0 1px #7f54b3; }
            .op-wc-orders .button:not(.button-primary) { border-radius: 4px; }
            .op-wc-orders .search-box .button {
                background: #7f54b3; border-color: #7f54b3; color: #fff; border-radius: 4px;
            }
            .op-wc-orders .search-box .button:hover { background: #6b4794; border-color: #6b4794; color: #fff; }
        </style>
        <div class="wrap op-wc-orders">
            <h1 class="wp-heading-inline"><?php echo esc_html__( 'POS Orders', 'openpos' ); ?></h1>
            <hr class="wp-header-end">
            <form method="get">
                <input type="hidden" name="page" value="op-orders" />
                <?php
                    if ( ! empty( $_GET['register'] ) ) {
                        echo '<input type="hidden" name="register" value="' . esc_attr( (int) $_GET['register'] ) . '" />';
                    }
                    ...
                    $list_table->search_box( __( 'Search', 'openpos' ), 'op-order' );
                    $list_table->display();
                ?>
            </form>
        </div>
    ...
    public function setting_page()
    {
        ?>
        <style type="text/css">
            /* ---- WooCommerce-style polish for POS Settings (tabs + cards + form rows) ---- */
            .op-wc-setting { margin-right: 20px; }
            .op-wc-setting .op-wrap, .op-wc-setting.op-wrap { max-width: 1100px; }

            /* Tabs */
            .op-wc-setting .nav-tab-wrapper, .op-wc-setting .metabox-holder .group + .group { }
            .op-wc-setting .nav-tab-wrapper { border-bottom: 1px solid #dcdcde; margin: 0 0 20px; padding: 0; }
            .op-wc-setting .nav-tab {
                background: transparent; border: 0; border-bottom: 2px solid transparent;
                margin: 0 4px -1px 0; padding: 12px 18px; font-size: 13px; font-weight: 600;
                color: #50575e; line-height: 1.4; box-shadow: none;
            }
            .op-wc-setting .nav-tab:hover { background: #f6f7f7; color: #7f54b3; }
            .op-wc-setting .nav-tab-active, .op-wc-setting .nav-tab-active:focus, .op-wc-setting .nav-tab-active:hover {
                background: transparent; color: #7f54b3; border-bottom-color: #7f54b3;
            }

            /* Kill the outer wrapper frame so only the card shows (no nested boxes) */
            .op-wc-setting .metabox-holder, .op-wc-setting .openpos-metabox-holder {
                border: 0; background: transparent; box-shadow: none; padding: 0; margin: 0;
            }

            /* Card */
            .op-wc-setting .metabox-holder .postbox, .op-wc-setting .group {
                border: 1px solid #e0e0e0; background: #fff;
                box-shadow: 0 1px 2px rgba(0,0,0,.04); padding: 4px 24px 20px; margin-bottom: 20px;
                overflow: hidden;
            }
            .op-wc-setting .group > form > h2,
            .op-wc-setting .group > form > h3,
            .op-wc-setting .group > h2, .op-wc-setting .group > h3 {
                font-size: 15px; font-weight: 600; color: #1e1e1e;
                margin: 12px 0 0; padding: 8px 0 16px; border: 0;
            }

            /* Form rows: kill global zebra, use clean WooCommerce row separators */
            .op-wc-setting .form-table { margin: 0; border-collapse: collapse; }
            .op-wc-setting .form-table tr:nth-child(odd),
            .op-wc-setting .form-table tr:nth-child(even),
            .op-wc-setting .form-table tr { background: transparent !important; }
            .op-wc-setting .form-table > tbody > tr { border-top: 1px solid #f0f0f1; }
            .op-wc-setting .form-table > tbody > tr:first-child { border-top: 0; }
            .op-wc-setting .form-table th {
                color: #1e1e1e; font-weight: 600; font-size: 13px;
                width: 230px; padding: 22px 16px 22px 0; vertical-align: top; line-height: 1.5;
            }
            .op-wc-setting .form-table td { padding: 18px 0; vertical-align: top; }
            .op-wc-setting .form-table td p.description,
            .op-wc-setting .form-table td .description,
            .op-wc-setting .form-table td > p {
                color: #757575; font-size: 12.5px; font-style: normal; margin: 8px 0 0; max-width: 640px;
            }

            /* Inputs */
            .op-wc-setting .form-table input[type="text"],
            .op-wc-setting .form-table input[type="number"],
            .op-wc-setting .form-table input[type="email"],
            .op-wc-setting .form-table input[type="url"],
            .op-wc-setting .form-table input[type="password"],
            .op-wc-setting .form-table select,
            .op-wc-setting .form-table textarea {
                border: 1px solid #c3c4c7; border-radius: 4px; min-height: 34px; box-shadow: none;
            }
            .op-wc-setting input[type="text"]:focus, .op-wc-setting input[type="number"]:focus,
            .op-wc-setting input[type="email"]:focus, .op-wc-setting input[type="url"]:focus,
            .op-wc-setting select:focus, .op-wc-setting textarea:focus {
                border-color: #7f54b3; box-shadow: 0 0 0 1px #7f54b3; outline: none;
            }

            /* Save button row */
            .op-wc-setting p.submit { margin: 0; padding: 20px 0 4px; border-top: 1px solid #f0f0f1; }
            .op-wc-setting .button-primary {
                background: #7f54b3; border-color: #7f54b3; color: #fff; border-radius: 4px;
                box-shadow: none; text-shadow: none; min-height: 36px; padding: 0 16px; line-height: 34px;
            }
            .op-wc-setting .button-primary:hover, .op-wc-setting .button-primary:focus {
                background: #6b4794; border-color: #6b4794; color: #fff; box-shadow: 0 0 0 1px #6b4794;
            }
        </style>
        <div class="op-wrap wrap op-admin-wrap op-wc-setting">
        <?php
                ...
                try{
                    ...
                    switch ($report_type)
                    {
                        ...
                        case 'sale_by_payment':
                            ...
                            foreach($orders as $_order_id)
                            {
                                if ( $_order_id instanceof WC_Order )
                                {
                                	$order = $_order_id;
                                    $_order_id = $_order_id->get_id();
                                }else{
                                ...
                                $order_id_list[] = array('order_id' => $_order_id,'local_id' => $_op_local_id);
                            }
                            $transactions_map = $op_transaction->getOrderTransactionsMetaBatch($order_id_list);
                            $all_transaction_ids = array_unique(
                                array_merge( ...array_values( $transactions_map ) )
                            );
                            foreach ( $all_transaction_ids as $transaction_id ) {
                                $transaction = $op_transaction->get( $transaction_id );
                                if ( empty( $transaction ) ) continue;

                                $source_data = isset($transaction['source_data'] ) ? $transaction['source_data'] : array();
                                $_source_data_order_id = 0;
                                if(isset($source_data['order_id']) && $source_data['order_id'])
                                {
                                    $_source_data_order_id = $source_data['order_id'];
                                }
                                
                                if( $_source_data_order_id&& isset($source_data['order_local_id']) && $source_data['order_local_id'])
                                {
                                        $_source_data_order_id = $op_woo_order->get_order_id_from_local_id($source_data['order_local_id']);
                                }
                                if(!$_source_data_order_id && isset($source_data['order_number']) && $source_data['order_number'])
                                {
                                    $_source_data_order_id = $op_woo_order->get_order_id_from_number( $source_data['order_number'] );
                                }
                                $transaction['source_order_id'] = $_source_data_order_id;
                            ...
                            if($report_payment)
                            {
                                ...
                                foreach($transactions as $_transaction)
                                {
                                    $user_id = $_transaction['created_by_id'];
                                    $_source_data_order_id = $_transaction['source_order_id'];
                                    if(!in_array($_source_data_order_id,$all_order_ids))
                                    {
                                        $all_order_ids[] = $_source_data_order_id;
                                    }
                                    $payment_method = array(
                                        'payment_code' => $_transaction['payment_code'],
                                        'in_amount' => $_transaction['in_amount'],
                                        'out_amount' => $_transaction['out_amount'],
                                    );
                                    $created_at_time = $_transaction['created_at_time'];
                                    if($created_at_time)
                                    {
                                        $created_at_time = round($created_at_time / 1000);
                                    }
                                ...
                                $summary_data['total_order'] = count( $all_order_ids );
    ...
    public function report_page(){
        ...
        echo '<div class="wrap op-admin-wrap op-wc-reports">';
        echo '<h1 class="wp-heading-inline">'.esc_html__( 'POS Reports', 'openpos' ).'</h1><hr class="wp-header-end">';
    ...
    public function _order_barcode_func($atts)
    {
        ...
        $img_data = $this->core->generateBarcode($barcode, $barcode_mode,(int)$barcode_width,(int)$barcode_height);
