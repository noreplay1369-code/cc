...
if(!class_exists('OP_Report'))
{
    class OP_Report{
        ...
        function op_report_result($result,$ranges,$report_type){
            ...
            return  apply_filters('class_op_report_result',$result,$ranges,$report_type,$this);
        ...
        public function print_xreport(){
            ...
            if($post && $post->post_type == 'op_z_report')
            {
                ...
                $cash_counter = isset($json_data['cash_counter']) ? $json_data['cash_counter'] : false;
                ...
                $template_id = get_post_meta($login_cashdrawer_id,'_op_zreport_template',true,$post);
                if(!$template_id)
                {
                    $template_id = 0;
                }
                ...
                if( $use_register_z_template )
                {
                    ...
                    foreach($transaction_data as $transaction_id)
                    {
                        ...
                        if(isset($transaction_details['source_type']) && $transaction_details['source_type'] == 'refund_order'){
                            $refund_total += $out_amount;
                        }
                        if(isset($transaction_details['source_type']) && ($transaction_details['source_type'] == 'order' || $transaction_details['source_type'] == 'refund_order'))
                        {
        ...
        public function print_zreport(){
            ...
            try{
                ...
                foreach($register_report as $date_label => $_posts)
                {
                        ...
                        foreach($_posts as $p)
                        {
                            ...
                            if(is_array($zreport_data) && !empty($zreport_data))
                            {
                                ...
                                if(isset($zreport_data['sale_payments']) && is_array($zreport_data['sale_payments']))
                                {
                                    foreach($zreport_data['sale_payments'] as $payment_code => $payment_data)
                                    {
                                        if(isset($sale_payments[$payment_code]))
                                        {
                                            $sale_payments[$payment_code]['total'] += $payment_data['total'];
                                            $sale_payments[$payment_code]['base_total'] += $payment_data['base_total'];
                                            $sale_payments[$payment_code]['refund_total'] += $payment_data['refund_total'];
                                        }else{
                                            $sale_payments[$payment_code] = $payment_data;
                                        }
                                    }
                                }
                                if(isset($zreport_data['taxes']) && is_array($zreport_data['taxes']))
                                {
                                   
                                    foreach($zreport_data['taxes'] as  $tax_data)
                                    {
                                        $tax_code = $tax_data['code'];
                                        $tax_total += $tax_data['total'];
                                        if(isset($taxes[$tax_code]))
                                        {
                                            $taxes[$tax_code]['total'] += $tax_data['total'];
                                        }else{
                                            $taxes[$tax_code] = $tax_data;
                                        }
                                    }
                                }
                                $tip_total += isset($zreport_data['tip_total']) ? 1 * $zreport_data['tip_total'] : 0;
                            }
                        }
                        if($logged_in_time){
                            $login_date .= '<br/><small><i>'.get_date_from_gmt($logged_in_time,'d-m-Y H:i:s').'</i></small>';
                ...
                $template = apply_filters('op_z_report_template',OPENPOS_DIR.'templates/admin/report/print_z_report.php',$register_report ,$posts,$register,$table_data);
                require($template);
                exit;
            }catch(Exception $e)
            {
